...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["none", "online"],
        help="Enable online routing analysis during inference server rollout.",
    )
    parser.add_argument("--interpretability_output_dir", type=str, default=None)
    parser.add_argument("--interpretability_model_label", type=str, default=None)
    parser.add_argument("--interpretability_flush_steps", type=int, default=None)
...
class Pi0TrainerConfig(_Pi0TrainerConfig):
    ...
    output_dir: str = field(
        default=f"/mlp_vepfs/share/yjh/checkpoints/mtvla7/libero/{moe_type}/8_prompt_{datetime.now().strftime('%m%d')}"
    )
    wandb_project: str = field(default='mtvla7')
...
class Pi0ModelConfig(_Pi0ModelConfig):
    ...
    moe_weight_path: Optional[str] = field(default="/mlp_vepfs/share/yjh/checkpoints/dexbotic/libero_all_pi0_ablation_raw_backward/all-0120/model-00003-of-00003.safetensors")
...
class Pi0InferenceConfig(_Pi0InferenceConfig):
    model_name_or_path: Optional[str] = field(
        default="/mlp_vepfs/share/yjh/checkpoints/mtvla7/libero/moe-lora/8_prompt_0209")
...
if __name__ == "__main__":
    ...
    for attr in (
        ...
        "interpretability_model_label",
        "interpretability_flush_steps",
    ):
        value = getattr(args, attr, None)
        if value is not None:
            setattr(exp.inference_config, attr, value)
    if args.task == "train":
        exp.train()
