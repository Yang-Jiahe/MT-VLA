...
class DexDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.report_task_prompts = bool(
            getattr(data_args, "report_task_prompts", False)
        )
        self.task_id_from_prompt = bool(
            getattr(data_args, "task_id_from_prompt", False)
        )
        self.task_prompt_max_items = int(
            getattr(data_args, "task_prompt_max_items", 50)
        )
        self.task_prompt_to_id = {}
        self.file_prompt_map = {}
        if self.report_task_prompts or self.task_id_from_prompt:
        ...
        for dataset_info in self.datasets_info:
            ...
            for jsonl_file, num_samples in sampled_data_index:
                if jsonl_file not in file_name_map:
                    ...
                    file_id += 1
                if jsonl_file not in file_samples:
                    file_samples[jsonl_file] = num_samples
    ...
    def unsafe_getitem(self, idx) -> dict:
        ...
        if self.task_id_from_prompt and self.file_prompt_map:
            prompt = self.file_prompt_map.get(jsonl_file)
            if prompt is not None:
                task_id = self.task_prompt_to_id.get(prompt)
    ...
    def _init_task_prompts(self) -> None:
        prompts = set()
        file_prompt_map = {}
        prompt_episode_counts = {}
        prompt_sample_counts = {}
        for jsonl_file in self.file_name_map.values():
            prompt = self._read_prompt_from_jsonl(jsonl_file)
            if not prompt:
                continue
            file_prompt_map[jsonl_file] = prompt
            prompts.add(prompt)
            prompt_episode_counts[prompt] = prompt_episode_counts.get(prompt, 0) + 1
            prompt_sample_counts[prompt] = (
                prompt_sample_counts.get(prompt, 0)
                + int(self.file_samples.get(jsonl_file, 0))
            )
        prompt_list = sorted(prompts)
        self.task_prompt_to_id = {prompt: idx for idx, prompt in enumerate(prompt_list)}
        self.file_prompt_map = file_prompt_map
        local_rank = int(os.getenv("LOCAL_RANK", "0") or 0)
        if self.report_task_prompts and local_rank == 0:
            logger.info("[data] Detected {} task prompts.", len(prompt_list))
            max_items = max(self.task_prompt_max_items, 0)
            for idx, prompt in enumerate(prompt_list[:max_items]):
                episode_count = int(prompt_episode_counts.get(prompt, 0))
                sample_count = int(prompt_sample_counts.get(prompt, 0))
                logger.info("[data] task_id={} prompt={}", idx, prompt)
                logger.info(
                    "[data] task_id={} episodes={} samples={}",
                    idx,
                    episode_count,
                    sample_count,
                )
            if len(prompt_list) > max_items:
                logger.info(
                    "[data] ... {} more prompts",
                    len(prompt_list) - max_items,
                )

    def _read_prompt_from_jsonl(self, jsonl_file: str) -> str | None:
        try:
            with megfile.smart_open(jsonl_file, "r") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue
                    return self._extract_prompt_from_item(item)
        except Exception:
            return None
        return None

    def _extract_prompt_from_item(self, item: dict) -> str | None:
        ...
        for key in (
            ...
            if key in item:
                return self._normalize_prompt_value(item[key])
        if "conversations" in item:
            return self._normalize_prompt_value(item["conversations"])
        return None

    def _normalize_prompt_value(self, value) -> str | None:
        if value is None:
            return None
        if isinstance(value, list):
            if not value:
                return None
            return self._normalize_prompt_value(value[0])
        if isinstance(value, dict):
            if "value" in value:
                return self._normalize_prompt_value(value["value"])
            return None
