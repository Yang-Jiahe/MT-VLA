    ...
    def _load_video(video_url, frame_indices):
        ...
        try:
            if os.path.isfile(video_url):
                vr = VideoReader(video_url, num_threads=1)
            else:
            ...
            if max(frame_indices) >= len(vr):
                raise IndexError(
                    f"Requested frame {max(frame_indices)} but video has {len(vr)} frames"
                )
            ...
            try:
                return LoadMultiModal._load_video_with_av(video_url, frame_indices)
            except Exception as av_exc:
                exists = os.path.exists(video_url)
                size = os.path.getsize(video_url) if exists and os.path.isfile(video_url) else None
                raise RuntimeError(
                    f"Failed to load video {video_url} at frame indices "
                    f"{frame_indices[:10]}; exists={exists}; size={size}; "
                    f"reader_frames={len(vr) if vr is not None else None}; "
                    f"decord_error={type(exc).__name__}: {exc}; "
                    f"av_error={type(av_exc).__name__}: {av_exc}"
                ) from exc
        return images

    @staticmethod
    def _load_video_with_av(video_url, frame_indices):
        frame_indices_set = set(frame_indices)
        images = {}
        if os.path.isfile(video_url):
            container = av.open(video_url)
        else:
            f = megfile.smart_open(video_url, mode='rb')
            container = av.open(f)
        try:
            stream = container.streams.video[0]
            for frame_idx, frame in enumerate(container.decode(stream)):
                if frame_idx > max(frame_indices_set):
                    break
                if frame_idx in frame_indices_set:
                    images[frame_idx] = Image.fromarray(frame.to_ndarray(format='rgb24'))
            missing_indices = frame_indices_set.difference(images)
            if missing_indices:
                raise IndexError(
                    f"Video ended before frames {sorted(missing_indices)[:10]}"
                )
        finally:
            container.close()
